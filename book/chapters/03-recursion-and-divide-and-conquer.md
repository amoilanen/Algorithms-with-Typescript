...
Now comes the key geometric insight. Sort the points in the strip by $y$-coordinate. For any point $p$ in the strip, how many other strip points can lie within distance $\delta$ of $p$? Geometrically, such points could be anywhere in a $2\delta \times 2\delta$ box centered on $p$ (width $2\delta$ from the strip, height $2\delta$ because points could be above or below). But we can cut this box in half with a simple algorithmic trick: we will design the inner loop so that it walks through the strip points in ascending $y$-order, and for each point $p$ it only checks points that come _after_ $p$ in that order — that is, points _above_ $p$. There is no need to look _below_ $p$, because any pair $(q, p)$ where $q$ is below $p$ will already have been examined when $q$ was the current point and $p$ was above it. This way, every pair in the strip is considered exactly once.
